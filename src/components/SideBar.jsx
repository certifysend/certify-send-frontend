...
const Sidebar = ({ activePage, setActivePage, onAddForm }) => {
  ...
  const handleMenuItemClick = (itemName) => {
    ...
    // If on mobile, collapse sidebar when adding a form
    if (itemName === "Forms" && window.innerWidth < 768) {
      if (onAddForm) onAddForm()
    }
  }
      ...
      >
                ...
                <li key={item.name}>
                  <button
                    className={`w-full flex items-center p-3 rounded-lg transition-colors duration-200
                      ${
                        activePage === item.name ? "bg-[#F2F4FF] text-[#5570F1]" : "text-gray-700 hover:bg-gray-200"
        ...
        <div className="p-2 space-y-2 border-t border-gray-100">
          ...
          <div className={`bg-[#F2F4FF] rounded-lg p-3 ${isCollapsed ? "text-center" : ""}`}>
            ...
            {!isCollapsed && <span className="text-xs text-[#5570F1] font-medium">Upgrade your account</span>}
          </div>
          ...
          <div className="flex items-center justify-between py-2 px-1">
            ...
            <button className={`p-1 rounded-full hover:bg-gray-200 ${isCollapsed ? "hidden" : ""}`} aria-label="Logout">
